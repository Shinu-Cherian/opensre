...
Session-local async investigation mode. Launch an RCA, keep using the shell, and get the finished report delivered to email, Telegram, Rocket.Chat, or Buzz.
...
| `/background notify set <channel[,channel...]>` | Set channels — `email`, `telegram`, `rocketchat`, `buzz`, or a combination |
...
Channels are **off by default** and reset on REPL restart. `email` requires the [`smtp`](/smtp) integration; `telegram` requires [`telegram`](/messaging/telegram); `rocketchat` requires [`rocketchat`](/messaging/rocketchat); `buzz` requires [`buzz`](/messaging/buzz). Interactive shell only — there is no `opensre background …` CLI command. Full guide: [Background investigations](/background-investigations).
