...
outcome = session.chat_until_goal(
    ...
    goal=SessionGoal(
        condition="Every active production alert rule has been checked.",
        checklist=(
...
The loop stops when the goal is achieved, cancelled, cleared, or its turn
budget is exhausted. The result contains the final `goal`, the `last_result`
from `chat`, and `turn_count`. Without `goal=`, the first action turn must
attach a goal; otherwise `chat_until_goal` returns after that one turn. Use
`cancel_requested` to stop between turns and `on_progress` to receive checklist
updates.
