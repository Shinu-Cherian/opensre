...
For production usage where integrations and session persistence are required, use `SessionManager` to bootstrap or resolve the session state:
...
# 3. Instantiate HeadlessAgent tied to the session state object
agent = HeadlessAgent(
    tools=NullToolProvider(),
    session=session,
)
