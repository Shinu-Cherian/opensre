...
| `OPENSRE_PROMPT_LOG_PATH` | `~/.opensre/prompt_log.jsonl` | Path to local prompt log file | `.env.example` |
| `OPENSRE_OPERATIONS_LOG_DISABLED` | `0` | Disable the local operations log | `.env.example` |
| `OPENSRE_OPERATIONS_LOG_PATH` | `~/.opensre/operations_log.jsonl` | Path to the local operations log | `.env.example` |
| `OPENSRE_OPERATIONS_LOG_MAX_BYTES` | `5242880` | Rotate the local operations log after this many bytes | `.env.example` |
...
| `OPENSRE_HOME` | `~/.opensre` | Root directory for local OpenSRE state, including sessions, settings, and operational files | `config/constants/paths` |
...
| `GRAFANA_LOKI_DATASOURCE_UID` | | UID of Loki datasource in Grafana (fallback when auto-discovery misses it) | `.env.example` |
| `GRAFANA_TEMPO_DATASOURCE_UID` | | UID of Tempo datasource in Grafana (fallback when auto-discovery misses it) | `.env.example` |
| `GRAFANA_MIMIR_DATASOURCE_UID` | | UID of Mimir/Prometheus datasource in Grafana (fallback when auto-discovery misses it) | `.env.example` |
...
| `SLACK_GATEWAY_INBOUND_TRANSPORT` | `socket_mode` | How Slack delivers events: `socket_mode` or `events_api_http` | `transports/slack/settings` |
| `SLACK_APP_TOKEN` | | App-level token (`xapp-…`). Required for `socket_mode` | `transports/slack/settings` |
| `SLACK_SIGNING_SECRET` | | Request-signature secret. Required for `events_api_http` | `transports/slack/settings` |
| `SLACK_GATEWAY_HTTP_PORT` | `3000` | Port the `events_api_http` listener binds | `transports/slack/settings` |
| `SLACK_GATEWAY_ALLOW_LOCAL_DEDUP` | | Accept process-local event dedup when `DATABASE_URL` is unset. Single replica only — a Slack retry reaching a second replica runs the turn twice | `transports/slack/startup` |
...
| `AZURE_LOG_ANALYTICS_ENDPOINT` | `https://api.loganalytics.io` | Log Analytics Query API host (override for sovereign clouds) | `integrations/_catalog_impl` |
...
| `AZURE_MAX_RESULTS` | `100` | Max Azure results (capped at 200) | `integrations/_catalog_impl` |
