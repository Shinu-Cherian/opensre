...
| **Implement** | satisfy a protocol the harness calls | `core.agent_harness.contracts` — `OutputSink`, `ToolProvider`, `PromptContextProvider`, `ReasoningClientProvider`, `RunRecordFactory`, `ErrorReporter`, `TurnAccounting`, `SessionState`, and the rebind mix-ins `ConsoleBindable` / `OutputBindable` / `SessionBindable` |
...
```python
from core.agent_harness.runtime import DefaultHeadlessBuild, GatherPhase, TurnBinding
...
agent = DefaultHeadlessBuild(
    ...
    gather=GatherPhase(),       # opt in to the evidence pass; default is disabled
)
...
The interactive shell shares the same `TurnHandler` → `SessionAgentPool` →
...
| Stream progress / persist tool calls of the evidence pass | two callables | `.agent(gather=GatherPhase(on_progress=…, persist=…))` |
